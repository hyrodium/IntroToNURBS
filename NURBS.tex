...
\end{align}
であり, まずは分母の一致のために次を充たす$w'_{i^1\cdots i^d}$を決定する.
\begin{align}
...
\end{align}
とすれば良い.
続いて分子の一致のために次を充たす$\bm{a}'_{i^1\cdots i^d}$を決定する.
\begin{align}
...
\end{align}
ここでも$w'_{i^1\cdots i^d}$と同様に考えて
\begin{align}
...
次の図\ref{Fig603}は$D_{22}$内でのノットを増やした際の模式図である.
\begin{figure}[H]
	\centering
	...
	\label{Fig603}
\end{figure}
$D_{22}$の上でのノットの密度を上げたかったのだが, $D_{12}, D_{21}$にも余計にノット列が挿入されている.
...
\begin{figure}[H]
	...
	\label{Fig604}
\end{figure}
つまり, $\supp(B_{i^1\cdots i^d})\subset D_{22}$を充たすような基底関数$B_{i^1\cdots i^d}$に関する制御点$\bm{a}_{i^1\cdots i^d}$だけを細分後に動かすこととし, それ以外は動かさないように決めておくのである.
このようにして$D_{12},D_{22}$上でのT-splineのパラメータ付けは細分後に変化しないようにでき, この領域に対応する制御点は細分前のものとする事で, 制御点を削減することが出来るのである.

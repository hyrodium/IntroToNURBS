...
$B_{(i,p)}$と$C_{(i,p)}$にはそれぞれの良い性質があり, $C_{(i,p)}$の良い性質とは上の証明で見た図\ref{Fig305}, 式\eqref{Eqn307}, \eqref{Eqn308} と等式$\int_\setR C_{(i,p)}dt=\sfrac{1}{(p+1)}$\footnote{後で使う訳ではないので未証明とした. 証明は簡単.}である.
...
\begin{proof}
    ...
    つまり, $\tilde{B}_{(i,j)}$は定理\ref{Thm101}と同じ漸化式を充たす.
	よって$\tilde{B}_{(i,j)}$と$B_{(i,j)}^{\text{B\'ez}}$は同じ漸化式を持つから, $\tilde{B}_{(i,j)}=B_{(i,j)}^{\text{B\'ez}}$である.
	故に
	\begin{align}
		B_{(i,j)}^{\text{B\'ez}}(t)=B_{(i-j+p,j)}^{\text{B-sp}}(t)
	\end{align}

...
\begin{tabbing}
    ...
    \> \labelitemi \, $\Z{i}{j}$ \> $i$から$j$までの整数の集合. $\Set{i,i+1,\dots,j-1,j}=\Squa{i,j}\cap\setZ$ \\
...
NURBS基底関数は, 後の有理基底関数やB-spline基底関数, 多重基底関数を寄せ集めたものであり, これらの基底関数はBernstein基底関数の一般化であるためである.
...
\begin{screen}
	...
	\end{defn}
\end{screen}
次の図\ref{Fig101}に$p\le 4$までのB\'ezier曲線の例を示す.\footnote{$p=0$では1点しか現れないが, これもB\'{e}zier曲線に含めるとする.}
\newpage
...
Bernstein基底関数について次の帰納的な定理が成り立つ.
\begin{screen}
	\begin{thm}
		\label{Thm101}
		Bernstein基底関数の帰納的性質
...
\end{screen}
\begin{proof}
	任意のAffine変換$T$は行列$A$による線形変換とベクトル$\bm{b}$の合成で表されるから
	\begin{align}
		\bm{p}(T(\bm{a}_i);t)
	...
	である.
\end{proof}
次の図\ref{Fig130}は$5$次B\'{e}zier曲線に対するAffine不変性の例を示している.
\begin{figure}[htbp]
	\centering
...
\noindent
Affine不変性は, 「制御点を平行移動や拡大縮小\footnote{Affine変換なので, 定理はより一般の変換での成立を主張している.}すれば曲線もそれに追従する」ことを意味しており, これは人間が制御点を直観的に決定することを容易にしている.
...
\begin{screen}
	...
	\end{defn}
\end{screen}
定理\ref{Thm201}より1の分割が充たされるから, 有理曲線もまたAffine不変性(定理\ref{affine})や凸包性(定理\ref{convex})を充たす.
...
\footnote{これによって写像$\mathbb{R}\to S^1$が得られるが, この写像は立体射影と本質的に同じである.}
\addtocounter{footnote}{-1}
\begin{figure}[htbp]
	\centering
...
定義式\eqref{Eqn301}の右辺の$B_{(i,p-1)},B_{(i+1,p-1)}$の係数は, それぞれ区間$\supp(B_{(i,p-1)}), \supp(B_{(i+1,p-1)})$上での1次関数であり, この区間上で$0$から$1$まで(或いは$1$から$0$まで)変化する.
(定理\ref{Thm301}参照.)
...
\newpage
次の定理はB-spline基底関数の性質の中でも特に重要である.
\begin{screen}
	\begin{thm}
		\label{Thm301}
		B-spline基底関数の台
...
\begin{screen}
	\begin{thm}
		...
		$B_{(i,p)}|_{[k_j,k_{j+1}]}$はノット列の部分列$ \pare{k_{j-p+1}, \dots ,k_{j+p}} $のみによって決定され, これは$ i $に依らない.
	\end{thm}
\end{screen}
\begin{proof}
...
\end{proof}
% \begin{proof}
% 	定理\ref{Thm301}より, B-sline基底関数の定義式\eqref{Eqn301}において, 右辺第一項の台は$ [k_{i}, k_{i+p}] $であり, 右辺第$2$項の台は$ [k_{i+1}, k_{i+p+1}] $である.
% 	これを考慮すれば,
% 	\UC
% \end{proof}
...
\begin{proof}
    ...
    $p-\N_{(i,p)}(t^*)=2$なる$t^*$が存在すれば, $B_{(i,p)}$は定数だから$\tau_{(i,p)}$は任意に取って問題ない.
    $p-\N_{(i,p)}(t^*)=1$なる$t^*$が存在すれば, $k_{i}<k_{i+1}=\cdots=k_{l-1}<k_{l}$であって, $t^*=k_{i+1}, B_{(i,p)}(t^*)=1$であるから, $\tau_{(i,p)}=k_{i+1}$とすれば良い.

    よって以降では, $p\ge 2$, $B_{(i,p)}\in C^1(\setR)$と仮定して良く, さらに強く$\tau_{(i,p)}\in \supp(B_{(i,p)})=\Squa{k_{i},k_{i+p+1}}$が存在して$\Squa{k_{i}, \tau_{(i,p)}}$で狭義単調増加, $\Squa{\tau_{(i,p)},k_{i+p+1}}$で狭義単調減少である事を示す.
